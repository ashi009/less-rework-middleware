...
/*!
 * Less - Rework - AutoPrefixer - middleware (adapted from the stylus middleware)
 *
 * Copyright(c) 2013 Randy Merrill <Zoramite+github@gmail.com>
 *
 * Extended by Xiaoyi Shi <ashi009@gmail.com>
 *
 * MIT Licensed
 */
...
var less = require('less'),
    ...
    moveMedia = require('rework-move-media'),
...
module.exports = less.middleware = function(options){
  ...
  var log = function(key, val, type) {
    if(options.debug || type === 'error') {
      ...
      console[type]('  \x1b[90m%s :\x1b[0m \x1b[36m%s\x1b[0m', key, val);
    }
  };
  ...
  // Auto prefixer versions
  options.autoprefixerVersions = options.autoprefixerVersions || ['last 2 versions', '> 1%', 'ie 7', 'ie 8'];

  // rework-move-media sorter
  options.moveMediaSorter = options.moveMediaSorter || null;
    ...
    parser.parse(str, function(err, tree) {
        ...
        try {
          css = rework(tree.toCSS())
              ...
              .use(removePrivateMixins)
              .use(removeDuplicateSelectors)
              .use(autoprefixer.apply(null, options.autoprefixerVersions).rework)
              .use(moveMedia(options.moveMediaSorter))
              .use(clearFix)
              .toString();
          ...
          if (compressOptions.compress || compressOptions.yuicompress) {
            ...
            imports[lessPath] = determine_imports(tree);
            callback(err, css);
          }
        } catch(parseError) {
...
function removePrivateMixins(sheet, rework) {
  // console.log(sheet.rules);
  sheet.rules = filterRules(sheet.rules);
  // console.log(sheet.rules);
  function filterRules(rules) {
    return rules.filter(function(rule) {
      if (rule.type === 'rule') {
        rule.selectors = rule.selectors.filter(noPrivate);
  ...
  function noPrivate(selector) {
    return selector.indexOf('.__') === -1;
  }
}

function removeDuplicateSelectors(sheet, rework) {
  processRules(sheet.rules);
  function processRules(rules) {
    rules.forEach(function(rule) {
      if (rule.type === 'rule') {
        var met = {};
        rule.selectors = rule.selectors.filter(function(selector) {
          if (met.hasOwnProperty(selector))
            return false;
          met[selector] = true;
          return true;
        });
